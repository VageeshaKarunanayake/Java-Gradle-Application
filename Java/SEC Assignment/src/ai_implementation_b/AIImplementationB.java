...
//GUIManage, RobotAI, RobotControl and RobotInfo interfaces imports
            ...
            while (true)
            {
                ...
                for (RobotInfo ROBOT : rc.getAllRobots())
                {
                    ...
                    if ((!ROBOT.getName().equals(myRobot.getName())) && (abs(myRobot.getX() - ROBOT.getX()) <= 2) && (abs(myRobot.getY() - ROBOT.getY()) <= 2) && (ROBOT.getHealth() != 0))
                    {
                        ...
                        //AIImplementationB thread will sleep for a half a second
                        Thread.sleep(500);
                        //Display the myRobot object's name with is firing
                        GM.writeLog(myRobot.getName()+" is firing\n");
                        //'Robot2Shoot' method will draw the line from 2nd robot to robot stored inside the 'ROBOT' object 
                        GM.Robot2Shoot(ROBOT.getX(), ROBOT.getY());
                        //Below method will refresh the GUI and display the accurate position and health
                        GM.refresh();
                ...
                else
                {
                    ...
                    Logger.getLogger("logger").info("AI ImplementationB interrupted! Exception - " + ex);
