package ai_implementation_a;


//Java imports 
import java.util.logging.Logger;

////imports of RobotInfo, RobotAI and GUIManage interfaces
import main_application.GUIManage;
import robot.RobotAI;
import robot.RobotControl;

//Below imports are needed if AI ImplementationA is implemented using Java
...
public class AIImplementationA implements RobotAI {
    ...
    //Declare native method called 'callCImplementation'  
    private native void callCImplementation(RobotControl rc, GUIManage GM);
    
    //static block to load the libarary called 'libCImplJNILibrary'
    static {
                System.loadLibrary("libCImplJNILibrary");
           }
    
    @Override
    public void runAI(RobotControl rc, GUIManage GM) {
        
        //Native method is called inside a try catch block to catch exceptions
        try
        {
        callCImplementation(rc,GM);
         } catch (Exception ex) {
                    Logger.getLogger("logger").info("AI ImplementationA interrupted! Exception - " + ex);
                } 
        
        
        /* This is the Java implementation of AIImplementationA class
        
        //Declare a string variable called 'direction' and initialized the variable with 'north' value
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if(!myRobot.isEmptyQueue())
                {
                    ...
                    //Display that the robot is moving east and move the robot to east
                    GM.writeLog(myRobot.getName()+" is moving East\n");
                    rc.moveEast();
                ...
                for (RobotInfo ROBOT : rc.getAllRobots())
                {
                    ...
                    if ((!ROBOT.getName().equals(myRobot.getName())) && (abs(myRobot.getX() - ROBOT.getX()) <= 2) && (abs(myRobot.getY() - ROBOT.getY()) <= 2) && (ROBOT.getHealth() != 0))
                    {
                        ...
                        //AIImplementationA thread will sleep for a half a second
                        Thread.sleep(500);
                        //Display the myRobot object's name with is firing
                        GM.writeLog(myRobot.getName()+" is firing\n");
                        //'Robot1Shoot' method will draw the line from 1st robot to robot stored inside the 'ROBOT' object
                        GM.Robot1Shoot(ROBOT.getX(), ROBOT.getY());
                        //Below method will refresh the GUI and display the accurate position and health
                        GM.refresh();
